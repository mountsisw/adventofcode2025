...
export default class Part1 extends PuzzlePart
{
    ...
    private columns : Map<number, Array<[number, number]>> = new Map();
    ...
    public calculateAnswer() : any
    {
        ...
        for (const row of this.rows.values()) row.sort((a, b) => a[0] - b[0]);
        for (const column of this.columns.values()) column.sort((a, b) => a[0] - b[0]);
    ...
    private recordPerimeter(tile1: Point, tile2: Point) : void
    {
        if (tile1.x === tile2.x)
        {
            const minY = Math.min(tile1.y, tile2.y);
            const maxY = Math.max(tile1.y, tile2.y);
            const column = this.columns.get(tile1.x) ?? new Array<[number, number]>();
        ...
        else if (tile1.y === tile2.y)
        {
            ...
            const row = this.rows.get(tile1.y) ?? new Array<[number, number]>();
            row.push([minX, maxX]);
            this.rows.set(tile1.y, row);
        }
    }

    private isSolidArea(corner1: Point, corner2: Point) : boolean
    {
        ...
        for (let coord = minX + 1; coord < maxX; coord++)
        {
            ...
            if (segments)
            {
                ...
                if (noOverlap === false) return false;
            }
        }
