...
class Part2 extends PuzzlePart {
    constructor() {
        super(...arguments);
    ...
    async calculateAnswer() {
        ...
        for (const machine of this.machines) {
            if (machine.increments.length > 5)
                continue;
            const pushCounter = new Array(machine.increments.length);
            pushCounter.fill(0);
            const maxPushesPerButton = machine.increments.map(increments => increments.reduce((min, increment, index) => increment === 0 ? min : Math.min(min, machine.targets[index]), Math.max(...machine.targets)));
            ...
            while (this.nextCombination(pushCounter, maxPushesPerButton, largestTarget)) {
                if (pushCounter.reduce((acc, count) => acc + count) >= largestTarget)
                    validCombos++;
    ...
    nextCombination(pushCounts, maxPushes, minPushes) {
        let index = 0;
        do {
    ...
    evaluateCounters(counters, targets) {
        if (counters.some((counter, index) => counter > targets[index]))
            return Part2.Comparisons.TOO_HIGH;
        if (counters.some((counter, index) => counter < targets[index]))
            return Part2.Comparisons.TOO_LOW;
        return Part2.Comparisons.JUST_RIGHT;
    }
}
Part2.Comparisons = { TOO_LOW: 0, JUST_RIGHT: 1, TOO_HIGH: 2 };
