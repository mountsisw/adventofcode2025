...
{
    private floorMap : Array<string> = new Array();

    public processRecord(record: string) : boolean
    {
        if (this.floorMap.length === 0) this.floorMap.push('.'.repeat(record.length + 2));
    ...
    public calculateAnswer() : any
    {
        ...
        {
            rollsAccessible = 0;
            const newFloorMap: Array<string> = new Array();
            ...
            for (let index = 1; index < this.floorMap.length - 1; index++)
            {
                ...
                for (let colIndex = 1; colIndex < thisRow.length - 1; colIndex++)
                {
                    ...
                    if (contents === '@')
                    {
                        ...
                        if (previousRow.charAt(colIndex - 1) === '@') adjoiningRolls++;
                        if (previousRow.charAt(colIndex) === '@') adjoiningRolls++;
                        if (previousRow.charAt(colIndex + 1) === '@') adjoiningRolls++;
                        if (thisRow.charAt(colIndex - 1) === '@') adjoiningRolls++;
                        if (thisRow.charAt(colIndex + 1) === '@') adjoiningRolls++;
                        if (nextRow.charAt(colIndex - 1) === '@') adjoiningRolls++;
                        if (nextRow.charAt(colIndex) === '@') adjoiningRolls++;
                        if (nextRow.charAt(colIndex + 1) === '@') adjoiningRolls++;
                        if (adjoiningRolls < 4) rollsAccessible++;
                        newFloorMap[index] += adjoiningRolls < 4 ? '.' : '@';
                    }
                    else newFloorMap[index] += contents;
                }
                newFloorMap[index] += '.';
            }
